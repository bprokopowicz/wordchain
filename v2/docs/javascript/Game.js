...
/*
    ...
    */
    ...
    getNextDisplayInstruction() {
        ...
        //TODO - need to distguish between good plays and mistakes.  
        const correct = true;
        ...
        if (this.wordToDisplayIndex < lastWordPlayedIndex) {
            ...
            displayInstruction = new DisplayInstruction(wordBeingDisplayed, Const.PLAYED, 0, correct, endOfGame);
            ...
            displayInstruction = this.displayInstructionForPlayingFromWordToWord(wordBeingDisplayed, nextWord);
        } else if (this.wordToDisplayIndex == highestIndex) {
            displayInstruction = new DisplayInstruction(this.fullSolutionGivenProgress.getTarget(), Const.TARGET, 0, true, false);
        } else {
            // we are displaying some word later than the currently active word, but not the taget.
            displayInstruction = new DisplayInstruction(wordBeingDisplayed, Const.FUTURE, 0, correct, endOfGame);
        }
    ...
    displayInstructionForPlayingFromWordToWord(lastWordPlayed, nextWord) {
        const correct = true;
        const endOfGame = false;
        if (nextWord.length == lastWordPlayed.length) {
            ...
            return new DisplayInstruction(lastWordPlayed, Const.CHANGE, changedCharIndex+1, correct, endOfGame);
        } else if (nextWord.length == lastWordPlayed.length+1) {
            // we display '+'s to let the user add a space
            return new DisplayInstruction(lastWordPlayed, Const.ADD_SPACE, 0, correct, endOfGame);
        } else if (nextWord.length == lastWordPlayed.length-1) {
            return new DisplayInstruction(lastWordPlayed, Const.DELETE, 0, correct, endOfGame);
        } else {
